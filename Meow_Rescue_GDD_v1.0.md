...
- **4가지 신규 메커니즘**: 벽 블록, 연결 블록, 포탈, 멀티캣으로 퍼즐 다양성 확보
...
| **고아 방지** | 생성 시 파트너 배치 실패하면 첫 번째 블록도 `removeBlock()`으로 제거 |
...
| **목적지 검사** | 워프 대상 셀이 비어있어야 텔레포트 발동 (BFS 4방향 모두 `buildGrid` + 점유 검사) |
| **생성 제약** | A≠B 보장 |
...
| **보상형 광고 간격** | 3 스테이지마다 |
| **보상형 광고 타이밍** | "Next Stage" 버튼 클릭 시 (클리어 직후가 아님) |
| **오프라인 방지** | 광고 미로드 시 Toast 안내 + 보상 차단 |
...
| **보상형 광고** | Next Stage / 힌트 / 자동풀이 | 시청 완료 시에만 보상 지급 |
...
스테이지 클리어 → 탈출 애니메이션 → 승리 오버레이 표시
  → 사용자가 "Next Stage" 버튼 클릭
    → shouldShowAd() 체크
      → 조건 충족 + 광고 로드됨: 보상형 광고 표시 → 시청 완료 → 다음 스테이지 로드
      → 조건 충족 + 광고 미로드: Toast 안내, 진행 차단
...
│                                       // 보상형 광고 간격 카운터 시스템 (SharedPreferences 영속)
└── util/
...
| `featuresForStageDistribution` | 스테이지별 피처 분포 정확성 (벽/연결/포탈/멀티캣 포함) |
...
| `newFeaturesStagesSolvable` | 51~200 스테이지 신규 메커니즘 풀이 가능성 |
