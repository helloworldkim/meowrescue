...
- **얼음 타일**: 블록이 멈출 수 없는 미끄러운 셀 — 장애물에 부딪힐 때까지 슬라이딩 (스테이지 71+)
- **연결 블록**: 같은 linkId를 가진 한 쌍의 블록이 동시에 이동 (스테이지 91+)
- **포탈**: 고양이 전용 워프 셀 — A에 도착하면 B로 텔레포트 (스테이지 111+)
...
- **얼음 타일**: 하늘색 ❄ 아이콘 셀. 블록이 얼음 위에서 멈출 수 없으며 벽이나 다른 블록에 부딪힐 때까지 계속 미끄러짐.
- **연결 블록**: 주황색 🔗 아이콘 블록 쌍. 한쪽을 드래그하면 파트너도 같은 방향·거리로 동시 이동. Undo 시 양쪽 모두 복원.
- **포탈**: 보라색(A)/청록색(B) 맥동 원형 셀. 고양이가 한쪽에 도착하면 반대편으로 즉시 텔레포트. Undo 시 원래 위치 복원.
...
- 벽 블록 자동 스킵, 연결 블록 쌍 동시 이동, 얼음 타일 중간 정지 금지
- 포탈 텔레포트: 고양이 이동 후 목적지 점유 검사 + 자동 워프
